...
from nlp import symptoms, word_extractor, noun_token_extractor, send_openai_request
import csv
import warnings
...
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.33,
                                                    random_state=42)  # Separa el dataset en train y test con un seed de 42
testx = testing[cols]
...
def print_disease(node):
    ...
    disease = le.inverse_transform(
        val[0])  # le hace el inverse transform para que te devuelva el nombre de la enfermedad
    return list(map(lambda x: x.strip(), list(
        disease)))  # devuelve la enfermedad, borra los espacios en blanco de las enfermedades para devolver un string decente


# metodo que hace el arbol de decision
def tree_to_code(tree, feature_names):
    # feature names es de tipo Index que tiene un PandasArray adentro.
    tree_ = tree.tree_
    feature_name = [
    ...
    '''
        feature name has length 147 while there are 132 symptoms. It includes "undefined!" items.
        chk_dis has length = 132, and it is a regular plain list of all the symptoms
    '''
    ...
    while True:
        ...
        #disease_input = input("")
        disease_input = "My head hurts" # hardocoded for debugging
        # TODO: aca hay que poner el intercepter de ortografia
        # checks the pattern of the input given by the user
        found_symptoms = send_openai_request(disease_input)
        ...
        # conf es un valor que determina si se encontro algun sintoma que matchea con la lista que tenemos o no. 1 si encontro, 0 si no.
        # TODO ver por aca como hacer para que se pueda matchear con varios. ver la linea que llama check_pattern()
        if conf == 1:
            print("searches related to input: ")
            for num, it in enumerate(cnf_dis):
                print(num, ")", it)
            if num != 0:
                # pregunta si hay mas de un sintoma matcheado cual es y que de una opcion entre 0 y la cantidad de sintomas
    ...
    while True:
        try:
            # num_days = int(input("Okay. From how many days ? : "))  COMENTADO PARA EL DEBUGGER
            num_days = 2
            break
        except:
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            if val <= threshold:  # si matchea con el sintoma va a la derecha y sino a la izquierda del arbol
                ...
                recurse(tree_.children_right[node],
                        depth + 1)  # si matchea con el sintoma va a la derecha y sino a la izquierda del arbol
        else:
            ...
            symptoms_exp = []
            for syms in list(
                    symptoms_given):  # Con al enfermedad posible agarra los sintomas y les pregunta si tiene alguno, si los tiene los apendea a la lista
                inp = ""
                print(syms, "? : ", end='')
                while True:
                    inp = random.choice(["yes", "no"])
                    #inp = input("")
            ...
            calc_condition(symptoms_exp,
                           num_days)  # calcula la condiciones de la enfermedad en base a los dias que le pasaste, y te dice si tenes que ver un doctor o no
            ...
            # print(description_list[present_disease[0]])
            precution_list = precautionDictionary[present_disease[0]] # TODO agregar las de la second prediction
